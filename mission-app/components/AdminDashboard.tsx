...
const AdminDashboard: React.FC = () => {
  ...
  const [viewMode, setViewMode] = useState<'LOBBY' | 'DASHBOARD'>('LOBBY');
  ...
  useEffect(() => {
    if (viewMode === 'DASHBOARD' && localRoom?.roomCode) {
      // 방 정보 실시간 구독
      const unsubRoom = subscribeToRoom(localRoom.roomCode, (room) => {
        if (room) setLocalRoom(room);
      });

      // 팀 정보 실시간 구독
      const unsubTeams = subscribeToTeams(localRoom.roomCode, (teamsData) => {
        setTeams(teamsData);
      });
  ...
  const handleCreateRoom = async () => {
    ...
    try {
      const code = await createRoom(newOrgName, newTeamCount);
      const room = await getRoom(code);
      const teamsData = await getTeams(code);
  ...
  const handleStartGame = async () => {
    if (!localRoom) return;
    await startGame(localRoom.roomCode);
  };

  const handleEndGame = async () => {
    if (!localRoom) return;
    await endGame(localRoom.roomCode);
    setShowResults(true);
  };
  ...
  const getSortedTeams = () => {
    return (Object.values(teams) as TeamData[]).sort((a, b) => {
      ...
      return (b.completedLocations?.length || 0) - (a.completedLocations?.length || 0);
    });
  };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="bg-imf-dark border border-gray-700 p-8 rounded-xl shadow-lg h-fit">
              ...
              <div className="space-y-6">
                ...
                >
                  <Activity size={20} /> {isCreating ? '생성 중...' : '작전 개시 (Create)'}
                </button>
              </div>
            </div>
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <div className="flex items-center gap-3 mt-2 text-xs font-mono text-gray-500">
                            ...
                            {room.isStarted && !room.isEnded && <span className="text-green-500 font-bold">LIVE</span>}
                  ...
                  <div className="flex justify-between items-start mb-4 border-b border-gray-800 pb-2">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1 text-sm bg-black/40 px-2 py-1 rounded">
                        ...
                        <span className="text-white font-bold">{team.members?.length || 0}명</span>
                        <span className="text-gray-500 text-xs truncate max-w-[200px]">
                          {team.members && team.members.length > 0 ? `(${team.members.join(', ')})` : '(대기 중)'}
                        </span>
                      </div>
                    </div>
