...
const UserJoin: React.FC<Props> = ({ room, onJoin, onBack }) => {
  ...
  useEffect(() => {
    ...
    if (localRegistry.length > 0) {
      setRecentRooms(localRegistry);
    } else {
      // UX Improvement: If no local rooms, default to Manual Input
      setShowManualInput(true);
    }

    // Subscribe to API for real-time cross-device room discovery
    const unsubscribe = subscribeToRooms((apiRooms) => {
      // Merge API rooms with local registry
      ...
      // API rooms take precedence (for cross-device discovery)
      apiRooms.forEach(room => mergedMap.set(room.roomCode, room));
      ...
      // If we now have rooms, we can show the list view
      if (merged.length > 0 && showManualInput) {
        // Keep manual input if user was already there
      }
    });
